...
import { User, Server, Shield, Info, Lock } from 'lucide-react';
...
const IPAMGrid: React.FC<IPAMGridProps> = ({ subnet, devices, customers, onNavigateToDevice, onNavigateToCustomer }) => {
  ...
  const getIPInfo = (octet: number) => {
    ...
    return { status: 'AVAILABLE', type: 'FREE', name: 'Available' };
  };
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex flex-wrap gap-4 text-xs border-b border-slate-100 pb-4">
          ...
          <div className="flex items-center gap-2">
              ...
              <span className="text-slate-600">Infrastructure (Device)</span>
          </div>
          <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-blue-100 border border-blue-300 rounded"></div>
              <span className="text-slate-600">Subscriber (Customer)</span>
          </div>
          <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-slate-800 border border-slate-900 rounded"></div>
              <span className="text-slate-600">Reserved / Gateway</span>
          </div>
      </div>
      ...
      <div className="grid grid-cols-8 sm:grid-cols-16 md:grid-cols-16 lg:grid-cols-32 gap-1.5">
          {ips.map(octet => {
              ...
              return (
                  <div 
                    key={octet}
                    className={`relative aspect-square rounded border flex items-center justify-center text-[10px] font-mono transition-all ${bgClass}`}
                    ...
                    onClick={() => {
                        ...
                        if (info.type === 'CUSTOMER' && info.data) onNavigateToCustomer((info.data as Customer).id);
                    }}
                  >
                      {octet}
                      ...
                      {hoveredIP === octet && (
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-slate-900 text-white text-xs p-3 rounded-lg shadow-xl z-10 pointer-events-none">
                              <div className="font-bold border-b border-slate-700 pb-1 mb-1 text-center">{subnet}.{octet}</div>
      ...
      <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
              ...
              <p className="opacity-80">
                  Static IPs are assigned manually by Network Engineers. Dynamic pools (PPPoE) are managed by the RADIUS server range .100 to .200.
              </p>
          </div>
      </div>
